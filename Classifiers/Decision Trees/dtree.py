...
def entropy(dataset):
    ...
    n = len(dataset)
    nPos = len([x for x in dataset if x.positive]) #001 
    nNeg = n - nPos
    if nPos == 0 or nNeg == 0:
        return 0.0
...
def prune(data, val):
    maxP = 0
    best = data
    trees = allPruned(data)
    for tree in trees:
        performance = check(tree, val)
        if(performance > maxP):
            maxP = performance
            best = tree          
    if maxP < check(data, val):
        return data
    else:
        return prune(best, val)

def averageScore(tree, test, numberOfTests):
    score = 0
    for i in range(0,numberOfTests):
        score += check(tree, test)
        print(check(tree, test))
    score = score/numberOfTests
    return score     

def listOfScores(tree, test, numberOfTests):
    scoreList = []
    score = 0
    for i in range(0, numberOfTests):
        score = check(tree, test)
